...
const receipt = {
  ...
  product_id: yearlyid, // external var
...
for (const i in list) { // NOTE: `list` must be provided externally
  const regex = new RegExp('^' + i, 'i');
  if (regex.test(ua) || regex.test(bundle_id)) { // `ua` and `bundle_id` external
    ...
    if (hx && hx.includes && hx.includes('hxpda')) {
    ...
    } else if (hx && hx.includes && hx.includes('hxpdb')) {
      ddm.receipt.in_app = data;
    } else if (hx && hx.includes && hx.includes('hxpdc')) {
      const xreceipt = {
...
if (!anchor) {
  // fallback branch (used when UA/bundle_id not matched)
  const order = '3|2|4|5|0|1'.split('|');
  let idx = 0;
  while (true) {
    switch (order[idx++]) {
      case '0':
